#!/usr/bin/env python3
"""
Script para migrar dados da aba "dfc_n1" do Excel para dfc_structure_n1
"""
...
def read_excel_dfc_n1_tab():
    """Lê a aba "dfc_n1" do Excel"""
    try:
        # Tentar diferentes nomes de aba
        sheet_names = ['dfc_n1', 'dfc-n1', 'dfc n1', 'DFC N1', 'DFC_N1']
...
def clean_and_transform_dfc_n1_data(df):
    ...
    column_mapping = {
        'dfc_n1_id': 'dfc_n1_id',  # order_index
        'dfc_n1': 'dfc_n1',        # description
    }
...
def insert_dfc_n1_structure_data(conn, df, grupo_empresa_id, empresa_ids):
    """Insere dados na tabela dfc_structure_n1"""
    cur = conn.cursor()
    ...
    try:
        ...
        for empresa_id in empresa_ids:
            cur.execute("""
                SELECT COUNT(*) FROM dfc_structure_n1 
                WHERE empresa_id = %s
            """, (empresa_id,))
        ...
        for index, row in df.iterrows():
            print(f"  📝 Processando linha {index + 1}: {row['dfc_n1']}")
            
            for empresa_id in empresa_ids:
                record_id = str(uuid.uuid4())
                
                # Tratar a coluna name removendo sinais operadores da description
                description = row['dfc_n1'] if pd.notna(row['dfc_n1']) else None
                ...
                insert_data = {
                    ...
                    'order_index': row['dfc_n1_id'] if pd.notna(row['dfc_n1_id']) else None,
...
def main():
    """Função principal"""
    print("🚀 MIGRANDO DADOS DA ABA 'dfc_n1' PARA dfc_structure_n1...")
    print("=" * 70)
    
    # Ler Excel
    df = read_excel_dfc_n1_tab()
    if df is None:
        return
    ...
    try:
        ...
        df_clean = clean_and_transform_dfc_n1_data(df)
        
        # Inserir dados
        if insert_dfc_n1_structure_data(conn, df_clean, grupo_empresa_id, empresa_ids):
