#!/usr/bin/env python3
"""
Script para migrar dados da aba "dfc_n2" do Excel para dfc_structure_n2
"""
...
def read_excel_dfc_n2_tab():
    """Lê a aba "dfc_n2" do Excel"""
    try:
        # Tentar diferentes nomes de aba
        sheet_names = ['dfc_n2', 'dfc-n2', 'dfc n2', 'DFC N2', 'DFC_N2']
...
def clean_and_transform_dfc_n2_data(df):
    ...
    column_mapping = {
        'dfc_n2_id': 'dfc_n2_id',      # order_index
        'dfc_n2': 'dfc_n2',            # description
        'dfc_n1_id': 'dfc_n1_id',      # dfc_n1_ordem
    }
...
def insert_dfc_n2_structure_data(conn, df, grupo_empresa_id, empresa_ids):
    """Insere dados na tabela dfc_structure_n2"""
    cur = conn.cursor()
    ...
    try:
        ...
        for empresa_id in empresa_ids:
            cur.execute("""
                SELECT COUNT(*) FROM dfc_structure_n2 
                WHERE empresa_id = %s
            """, (empresa_id,))
        ...
        for index, row in df.iterrows():
            print(f"  📝 Processando linha {index + 1}: {row['dfc_n2']}")
            
            for empresa_id in empresa_ids:
                record_id = str(uuid.uuid4())
                
                # Tratar a coluna name removendo sinais operadores da description
                description = row['dfc_n2'] if pd.notna(row['dfc_n2']) else None
                ...
                # Usar dfc_n1_id do Excel diretamente para dfc_n1_ordem
                dfc_n1_ordem = row['dfc_n1_id'] if pd.notna(row['dfc_n1_id']) else None
                
                insert_data = {
                    'id': record_id,
                    'empresa_id': empresa_id,
                    'grupo_empresa_id': grupo_empresa_id,
                    'order_index': row['dfc_n2_id'] if pd.notna(row['dfc_n2_id']) else None,
                    'dfc_n1_ordem': dfc_n1_ordem,
                ...
                print(f"    🔗 DFC N1 Ordem: {insert_data['dfc_n1_ordem']}")
                print(f"    📝 Name: {insert_data['name']}")
                print(f"    🔧 Operation Type: {insert_data['operation_type']}")
                
                insert_query = """
                    INSERT INTO dfc_structure_n2 (
                        id, empresa_id, grupo_empresa_id, order_index, dfc_n1_ordem, description, name, 
                        operation_type, is_active, created_at, updated_at
                    ) VALUES (
                        %(id)s, %(empresa_id)s, %(grupo_empresa_id)s, %(order_index)s, %(dfc_n1_ordem)s,
...
def main():
    """Função principal"""
    print("🚀 MIGRANDO DADOS DA ABA 'dfc_n2' PARA dfc_structure_n2...")
    print("=" * 70)
    
    # Ler Excel
    df = read_excel_dfc_n2_tab()
    if df is None:
        return
    ...
    try:
        ...
        df_clean = clean_and_transform_dfc_n2_data(df)
        
        # Inserir dados
        if insert_dfc_n2_structure_data(conn, df_clean, grupo_empresa_id, empresa_ids):
