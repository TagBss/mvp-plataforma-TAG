#!/usr/bin/env python3
"""
Script para migrar dados da aba "dre_n2" do Excel para dre_structure_n2
"""
...
def read_excel_dre_n2_tab():
    """Lê a aba "dre_n2" do Excel"""
    try:
        # Tentar diferentes nomes de aba
        sheet_names = ['dre_n2', 'dre-n2', 'dre n2', 'DRE N2', 'DRE_N2']
...
def clean_and_transform_dre_n2_data(df):
    ...
    column_mapping = {
        'dre_n2_id': 'dre_n2_id',  # order_index
        'dre_n2': 'dre_n2',        # description
    }
...
def insert_dre_n2_structure_data(conn, df, grupo_empresa_id, empresa_ids):
    """Insere dados na tabela dre_structure_n2"""
    cur = conn.cursor()
    ...
    try:
        ...
        for empresa_id in empresa_ids:
            cur.execute("""
                SELECT COUNT(*) FROM dre_structure_n2 
                WHERE empresa_id = %s
            """, (empresa_id,))
        ...
        for index, row in df.iterrows():
            print(f"  📝 Processando linha {index + 1}: {row['dre_n2']}")
            
            for empresa_id in empresa_ids:
                record_id = str(uuid.uuid4())
                
                # Tratar a coluna name removendo sinais operadores da description
                description = row['dre_n2'] if pd.notna(row['dre_n2']) else None
                ...
                # Usar dre_n1_id do Excel diretamente para dre_n1_ordem
                dre_n1_ordem = row['dre_n1_id'] if pd.notna(row['dre_n1_id']) else None
                
                insert_data = {
                    'id': record_id,
                    'empresa_id': empresa_id,
                    'grupo_empresa_id': grupo_empresa_id,
                    'order_index': row['dre_n2_id'] if pd.notna(row['dre_n2_id']) else None,
                    'dre_n1_ordem': dre_n1_ordem,
                ...
                print(f"    🔗 DRE N1 Ordem: {insert_data['dre_n1_ordem']}")
                print(f"    📝 Name: {insert_data['name']}")
                print(f"    🔧 Operation Type: {insert_data['operation_type']}")
                
                insert_query = """
                    INSERT INTO dre_structure_n2 (
                        id, empresa_id, grupo_empresa_id, order_index, dre_n1_ordem, description, name, 
                        operation_type, is_active, created_at, updated_at
                    ) VALUES (
                        %(id)s, %(empresa_id)s, %(grupo_empresa_id)s, %(order_index)s, %(dre_n1_ordem)s,
...
def main():
    """Função principal"""
    print("🚀 MIGRANDO DADOS DA ABA 'dre_n2' PARA dre_structure_n2...")
    print("=" * 70)
    
    # Ler Excel
    df = read_excel_dre_n2_tab()
    if df is None:
        return
    ...
    try:
        ...
        df_clean = clean_and_transform_dre_n2_data(df)
        
        # Inserir dados
        if insert_dre_n2_structure_data(conn, df_clean, grupo_empresa_id, empresa_ids):
